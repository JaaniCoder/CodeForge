...
const Home = () => {
    ...
    min-h-screen max-h-screen relative bg-secondary px-3 py-6 flex flex-col items-center justify-start gap-4 transition-all duration-200 ease-in-out`}>
            ...
            <div className='overflow-hidden w-full flex flex-col gap-4'>
                {/* logo */}
                <Link to={"/home"}>
                    <img src={Logo} alt="Logo" className='object-contain w-20 h-auto px-1 py-2' />
                </Link>
                {/* start coding */}
                <Link to={"/newProject"}>
        ...
        <div className='flex-1 min-h-screen max-h-screen overflow-y-scroll h-full flex flex-col items-start justify-start px-4 md:px-12 py-4 md:py-12'>
            ...
            <div className='w-full flex items-center justify-between gap-3'>
                ...
                <div className='bg-secondary w-full px-4 py-3 rounded-md flex items-center justify-center'>
                    ...
                    <input value={searchTerm} type='text' className='flex-1 px-4 py-1 text-xl bg-transparent outline-none border-none text-primaryText placeholder:text-gray-600' placeholder='Search Here...' onChange={(e) => dispatch(SET_SEARCH_TERM(e.target.value))} />
                </div>
                {/* Profile Section */}
                    {!user && (
